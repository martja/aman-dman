...
class TimelineScrollPane: JScrollPane(VERTICAL_SCROLLBAR_NEVER, HORIZONTAL_SCROLLBAR_AS_NEEDED) {
    init {
    ...
    fun insertTimeline(timelineConfig: TimelineConfig) {
        val tl = TimelineView(timelineConfig)
    ...
    fun updateRange(start: Instant, end: Instant) {
        ...
        items.components.forEach { component ->
            if (component is TimelineView) {
                component.updateSelectedViewRange(start, end)
            }
        }
    }
